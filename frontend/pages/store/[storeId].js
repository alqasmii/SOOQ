...
export default function Storefront() {
  ...
  useEffect(() => {
    if (storeId) {
      axios.get(`http://localhost:8080/api/stores/${storeId}`).then((res) => setStore(res.data));
      axios.get(`http://localhost:8080/api/products/store/${storeId}`).then((res) => setProducts(res.data));
    }
  }, [storeId]);

  if (!store) return <p className="text-center text-gray-500">Loading store...</p>;
